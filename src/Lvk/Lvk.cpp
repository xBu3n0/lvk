...
void Lvk::create_debug_messenger() {
    ...
    if (
        utils::create_debug_utils_messenger_ext(instance, &createInfo, nullptr, &this->debug_messenger) != VK_SUCCESS
    ) {
...
void Lvk::set_background_color(const std::string &name, float background_color[4]) {
...
void Lvk::clean_up() {
    if (enable_validation_layer) {
        utils::destroy_debug_utils_messenger_ext(
            this->instance,
